...
/// Manages the state of a ride acceptance operation.
///
...
class RideAcceptanceController extends ChangeNotifier {
  ...
  /// Attempts to accept a ride request on behalf of the currently authenticated driver.
  Future<bool> acceptRide(String rideId) async {
    ...
    if (user == null || user.role != UserRole.driver) {
      _state = const ViewState.error('You must be logged in as a driver to accept a ride.');
      notifyListeners();
      return false;
    }

    if (!_requestsController.isOnline) {
      _state = const ViewState.error('You must be online to accept a ride.');
      notifyListeners();
      return false;
    }
    ...
    _state = const ViewState.loading(message: 'Accepting ride...');
    notifyListeners();
    ...
    try {
      ...
      _state = const ViewState.success(null, message: 'Ride accepted successfully.');
      notifyListeners();
      return true;
      ...
      _state = const ViewState.error('An unexpected error occurred while accepting the ride.');
      notifyListeners();
      return false;
    }
  }
